...
class DynamicVideoEdit():
    ...
    def update_speeds(self, clip_list):
        ...
        videos.append(Video(speed=adjust_speed,
                            path="dynamic_test_output/my_video_{}.mp4".format(index)))  # TODO - change to temp file
        # edit last segment
        words = get_words("my_video_{}".format(index+1))
        adjust_speed = self.wpm/(words*(60/self.last_video_duration))
        videos.append(Video(speed=adjust_speed,
                            path="dynamic_test_output/my_video_{}.mp4".format(index+1)))
        # update speeds
        concatenate_videos(videos=videos,
                           output_file=f"dynamic_test_output/final_output_video.mp4")
        # test
